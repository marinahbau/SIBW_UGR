<?php
	...
	function getEvento($idEv){
		...
		$stmt = $mysqli->prepare("SELECT id,nombre, organizacion, fecha, descripcion, enlace, etiquetas FROM eventos WHERE id=?");
		$stmt->bind_param("i", $idEv);
		$stmt->execute();
		//$res = $mysqli->query("SELECT nombre, organizacion, fecha, descripcion, enlace FROM eventos WHERE id=" . $idEv);
		$res = $stmt->get_result();
		
		$evento = array('id'=>'unknown','nombre' => 'unknown', 'organizacion' => 'unknown', 'fecha' => 'unknown' , 'descripcion' => 'unknown', 'enlace' => 'unknown','etiquetas' => 'unknown' );
		
		if($res->num_rows > 0) {
			$row = $res->fetch_assoc();
			
			$evento = array('id'=>$row['id'],'nombre' => $row['nombre'], 'organizacion' => $row['organizacion'], 'fecha' => $row['fecha'], 'descripcion' => $row['descripcion'], 'enlace' => $row['enlace'],'etiquetas' => $row['etiquetas']);
	...
	function insertarEvento($nombre, $organizacion, $fecha, $contenido, $enlace , $etiquetas, $icono){

		$mysqli = conectar_bd();

		$sql = "INSERT INTO eventos(nombre,organizacion,fecha,descripcion,enlace, etiquetas, icono) VALUES ('$nombre','$organizacion','$fecha','$contenido','$enlace', '$etiquetas', '$icono') ";
